...
func main() {
	...
	// Swagger UI.
	// ВАЖНО: в Gin нельзя одновременно регистрировать /swagger и /swagger/*any:
	// catch-all маршрут конфликтует с обычным маршрутом и backend падает при старте.
	// Поэтому /api/v1/swagger без хвоста редиректится на index.html на уровне nginx,
	// а backend держит только wildcard-маршрут Swagger UI.
	...
	protected.Use(middleware.JWTMiddleware())
	{
		...
		{
			terminal.POST("/authorize", middleware.AdminOnly(), handlers.TerminalAuthorize)
			terminal.GET("/keys", middleware.AdminOnly(), handlers.GetKeysForTerminal)
		}
	}
